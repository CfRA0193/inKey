...
{    
    ...
    // Вектор изменения порядка следования столбцов бит
    char rgbMVVector[NBITS];
    ...
    // для изменения порядка следования столбцов бит -
...
void CInKey::RotateBitData(long int lSourceSize,
                           ...
                           char *prgbBitData, long int L, int mode)
{
    ...
    switch (mode)
    {
        ...
        {        
            ...
            {
                prgbBitData[(bitDataPosition ^ 0x01) * NBITS * lSourceSize + j]\
                    = prgbBitData[bitDataPosition * NBITS * lSourceSize + i];
            }
        ...
        {        
            ...
            for (i = (NBITS * lSourceSize - sPartIndex),\
                 j = (sPartIndex - 1); j >= 0; i++, j--)
            {
                prgbBitData[(bitDataPosition ^ 0x01) * NBITS * lSourceSize + j]\
                    = prgbBitData[bitDataPosition * NBITS * lSourceSize + i];
            }
